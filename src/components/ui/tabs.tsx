...
function TabsList({ className, ...props }: TabsPrimitive.ListProps & React.RefAttributes<TabsPrimitive.ListRef>) {
  return (
    <TabsPrimitive.List
      className={cn(
        'bg-transparent flex h-9 flex-row items-center justify-center rounded-lg p-[3px]',
...
function TabsTrigger({
  ...
  return (
    <TextClassContext.Provider
      value={cn(
        'text-content-primary text-body border-b-1 border-transparent py-2',
        value === props.value && 'border-content-primary',
      )}
    >
      <TabsPrimitive.Trigger
        className={cn(
          ...
          // props.value === value && 'bg-transparent border-b-2 border-content-primary',
          className,
        )}
        {...props}
      >
