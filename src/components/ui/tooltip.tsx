        ...
        <TooltipPrimitive.Overlay style={Platform.select({ native: StyleSheet.absoluteFill })}>
          ...
          >
            <TextClassContext.Provider
              value="text-small text-content-tertiary" //text-xs text-primary-foreground
            >
              <TooltipPrimitive.Content
                sideOffset={sideOffset}
                className={cn(
                  // 'bg-primary z-50 rounded-md px-3 py-2 sm:py-1.5',
                  'bg-fill-secondary z-50 rounded-xl px-3 py-2 sm:py-1.5',
                  'border border-border-quaternary shadow-xl',
                  Platform.select({
                    web: cn(
